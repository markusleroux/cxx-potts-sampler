...
class ModelTest : public Model {
		public:
				ModelTest(int n, int q, int Delta, long double B,
				          const std::string &type) : Model(n, q, Delta, B, type) {}
...
class UpdateTest : public Update {
		public:
				static int bs_uniformSample_p(const BoundingList &bs) { return bs_uniformSample(bs); }
...
class CompressTest : public Compress {
		public:
				CompressTest(Model &model, int v, const BoundingList &bs_A) : Compress(model, v, bs_A) {}
				int getC1() { return c1; }
};
