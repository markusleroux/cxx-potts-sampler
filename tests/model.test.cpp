...
TEST_CASE("model class", "[Model]") {
  ModelTest mdl(5, 7, 3, 0.95, "cycle");
  SECTION("initialization") {
  ...
  SECTION("methods") {
    ...
    SECTION("generate a set A") {
      CHECK(mdl.bs_generateA_p(0, 3) ==
            BoundingList(7, std::vector<int>{0, 1, 2}));

      mdl.setBoundingList_p(2, std::vector<int>{1, 2});
      mdl.setBoundingList_p(4, std::vector<int>{2, 3});
      CHECK(mdl.bs_generateA_p(3, 5) ==
            BoundingList(7, std::vector<int>{0, 1, 2, 3, 4}));
    }
