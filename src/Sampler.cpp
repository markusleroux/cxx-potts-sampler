...
int Sampler::generateT(const Model &m) {
    return m.getSize() + 1 \
         + m.getEdgeCount() \
         + (int)(pow(m.getSize(), 2) * (m.q - m.Delta * (1 - m.B) / (m.q - m.Delta * (3 - m.B))));
}
...
bool Sampler::boundingChainIsConstant() const {
    ...
    return std::all_of(boundingChain.begin(), boundingChain.end(), [](const BoundingList &bs) { return bs.count() == 1; });
}
